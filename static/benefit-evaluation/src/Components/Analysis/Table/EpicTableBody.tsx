  ...
  let rows: RowType[] = items.map((item, _): RowType => {
    ...
    const cost: number = showMonetary
      ? upperIsMonetary
        ? item.issueCost!!.cost
        : Math.round(
            costValue * (item.issueCost!!.balanced_points || 0) * 100 * 100
          ) / 100
      : Math.round(
          (item.issueCost!!.balanced_points || 1 / items.length) * 100 * 100
        ) / 100;
        ...
        },
        {
          ...
          content: (
            <Tooltip content={"Cost"}>
              <Lozenge appearance="removed">{`${cost.toLocaleString("en-US")} ${
                item.balancedPoints!!.type === balancedPointsEnum.MONETARY ||
                showMonetary
                  ? postfix
                  : ""
